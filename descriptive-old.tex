...
\item Qualitative nominal.
\item Quantitative continuous.
\item Qualitative ordinal.
...
% @
% \begin{enumerate*}
% \item $\bar x=\Sexpr{mean(x)}$ injuries.
% \item $Me=\Sexpr{median(x)}$ injury.
% \item $Mo=\Sexpr{Mode(x)}$ injury.
% \item $Q_1=1$ injury, $Q_2=\Sexpr{quantile(x)[3]}$ injury and $Q_3=\Sexpr{quantile(x)[4]}$ injuries.
% \item $P_{32}=\Sexpr{quantile(x,0.32)}$ injury.
% \end{enumerate*}
% \end{sol}
...
% \item
% \item
% \item $\bar x=\Sexpr{mean(x)}$ min, $s=\Sexpr{round(stdev(x),4)}$ min and $cv=\Sexpr{cv(x)}$.
% \end{enumerate*}
% \end{sol}
